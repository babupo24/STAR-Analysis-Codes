...
void Run12pp200Ana::Loop()
{
	...
	// for (Long64_t jentry = 0; jentry < 10000; jentry++)
	{
		...
		for (int tr = 0; tr < fmaxpar; tr++)
		{
			...
			if (fpT[tr] > 0.5 && fpT[tr] < 15. && ffitPts[tr] > 15 && abs(feta[tr]) < 1. && fitPtsRatio_tr > .51 && fabs(fnSigmaPion[tr]) < 10.0)
			{
				// Track 2 loop
				for (int tr2 = tr + 1; tr2 < fmaxpar; tr2++)
				{
					Double_t fitPtsRatio_tr2 = (Double_t)ffitPts[tr2] / (Double_t)ffitPtsPoss[tr2];
					if (fpT[tr2] > 0.5 && fpT[tr] < 15 && ffitPts[tr2] > 15 && fabs(feta[tr2]) < 1. && fabs(fnSigmaPion[tr2]) < 10.0 && fitPtsRatio_tr2 > .51)
					{
