...
void drawMatchingFractionX()
{
    ...
    TH1D *hrecJP0_All = (TH1D *)fembed->Get("hMRecJP0Allx");
    TH1D *hrecJP1_All = (TH1D *)fembed->Get("hMRecJP1Allx");
    TH1D *hrecJP2_All = (TH1D *)fembed->Get("hMRecJP2Allx");
    // numerator (gen&rec)
    TH1D *hrecJP0 = (TH1D *)fembed->Get("hMRecJP0x");
    TH1D *hrecJP1 = (TH1D *)fembed->Get("hMRecJP1x");
    TH1D *hrecJP2 = (TH1D *)fembed->Get("hMRecJP2x");
    ...
    TH1D *hMDatJP0 = (TH1D *)fdata->Get("hxsecMJP0Ctr"); // Data input for unfolding
    TH1D *hMDatJP0_Corr = (TH1D *)hMDatJP0->Clone();     // Data input for unfolding
    hMDatJP0_Corr->Multiply(hmratioJP0);

    TH1D *hMDatJP1 = (TH1D *)fdata->Get("hxsecMJP1Ctr"); // Data input for unfolding
    TH1D *hMDatJP1_Corr = (TH1D *)hMDatJP1->Clone();     // Data input for unfolding
    hMDatJP1_Corr->Multiply(hmratioJP1);
    TH1D *hMDatJP2 = (TH1D *)fdata->Get("hxsecMJP2Ctr"); // Data input for unfolding
    TH1D *hMDatJP2_Corr = (TH1D *)hMDatJP2->Clone();     // Data input for unfolding
    hMDatJP2_Corr->Multiply(hmratioJP2);

    cout << "Bin Content : " << hMDatJP2->GetBinContent(5) << "  " << hMDatJP2->GetBinError(5) << "  " << hMDatJP2_Corr->GetBinContent(5) << "  " << hMDatJP2_Corr->GetBinError(5) << endl;
    ...
    cmratio->Update();
    cmratio->SaveAs("./UnfoldingResults/matchingFractionAngbkgCorrectionXsecBins.pdf");
}
