...
void Run12pp200Ana::Loop()
{
	...
	hConeResInt = new TH2D("hConeResInt", "", 160, 0., 0.7, 160, 0, 0.7);
	hConeResInt1D = new TH1D("hConeResInt1D", "", 100, -2, 2);
	...
	for (int i = 0; i < 12; i++)
	{
		...
		hConeResLt7[i] = new TH1D(Form("hConeResLt7%i", i), "", 100, -2.0, 2.0);
		hConeResLt6[i] = new TH1D(Form("hConeResLt6%i", i), "", 100, -2.0, 2.0);
		hConeResLt5[i] = new TH1D(Form("hConeResLt5%i", i), "", 100, -2.0, 2.0);
		hConeResLt4[i] = new TH1D(Form("hConeResLt4%i", i), "", 100, -2.0, 2.0);
		hConeResLt3[i] = new TH1D(Form("hConeResLt3%i", i), "", 100, -2.0, 2.0);
	}
	...
	*/
	...
	// for (Long64_t jentry = 0; jentry < 100; jentry++)
	{
		...
		for (int tr = 0; tr < fmaxpar; tr++)
		{
			...
			if (isrec_tr1 && idTruth[tr] > 0 && fId_mc[tr] != -999 && (fId_mc[tr] == idTruth[tr])) // 2,3 = e+-, 5,6 = mu+-, 8,9=pi+-, 11,12= K+-, 14,15=ppbar
				ismc_tr1 = kTRUE;
			...
			for (int tr2 = tr + 1; tr2 < fmaxpar; tr2++)
			{
				...
				if (isrec_tr2 && idTruth[tr2] > 0 && fId_mc[tr2] != -999 && (fId_mc[tr2] == idTruth[tr2])) // 2,3 = e+-, 5,6 = mu+-, 8,9=pi+-, 11,12= K+-,
					ismc_tr2 = kTRUE;
				...
				if (Minv > 1.90 && Minv < 2.20 && partonicPtBin < 5.) // filter to "bad" event
					continue;
				...
				if (ismc_tr1 && ismc_tr2)
				{
					...
					// if (cone_mc == 0)
					// cout << "Found 000   " << Minv_mc << " " << cone_mc << " " << feta_mc[tr] << " " << feta_mc[tr2] << "  " << feta[tr] << " " << feta[tr2] << "  " << fphi_mc[tr] << " " << fphi_mc[tr2] << fphi[tr] << " " << fphi[tr2] << endl;

					hMDiffInt->Fill(Minv_mc - Minv, ptw);
					hConeResInt->Fill(cone, cone_mc, ptw);
					hConeResInt1D->Fill(cone_mc - cone, ptw);

					for (int i = 0; i < nBins; i++)
					{
						if (Minv >= nBinsEdges[i] && Minv < nBinsEdges[i + 1])
						{
							hMGenRec[i]->Fill(Minv_mc - Minv, ptw);
							hConeResLt7[i]->Fill(cone_mc - cone, ptw);
							if (cone < 0.6)
								hConeResLt6[i]->Fill(cone_mc - cone, ptw);
							if (cone < 0.5)
								hConeResLt5[i]->Fill(cone_mc - cone, ptw);
							if (cone < 0.4)
								hConeResLt4[i]->Fill(cone_mc - cone, ptw);
							if (cone < 0.3)
								hConeResLt3[i]->Fill(cone_mc - cone, ptw);
						}
					}
					if (Minv_mc < 4.0 && pT_pair_mc < 15.0 && fabs(eta_pair_mc) < 1.0)
					{
