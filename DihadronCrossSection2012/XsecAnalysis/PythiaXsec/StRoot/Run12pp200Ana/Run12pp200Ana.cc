...
void Run12pp200Ana::Loop()
{
	...
	Double_t crossSection[nptBin] = {9.0012e+00, 1.46253e+00, 3.54566e-01, 1.51622e-01, 2.49062e-02, 5.84527e-03, 2.30158e-03, 3.42755e-04, 4.57002e-05, 9.72535e-06, 4.69889e-07, 2.69202e-08, 1.43453e-09}; // in mb
	// cross section in pb
	double ptXsecPb[13] = {0};
	for (int i = 0; i < 13; i++)
	{
		ptXsecPb[i] = crossSection[i] * 1e9;
	}
	...
	for (int i = 0; i < nptBin; i++)
	{
		...
		hpythiaXsec[i] = new TH1D(Form("hpythiaXsec_%g_%g", partPtRange[i], partPtRange[i + 1]), "", xnBins, xnBinsEdges);
		hevProcessed[i] = new TH1D(Form("hevProcessed_%g_%g", partPtRange[i], partPtRange[i + 1]), "", 1, 0, 1);
	}
	hpythiaXsecAbs = new TH1D("hpythiaXsecAbs", "", xnBins, xnBinsEdges);

	double eventProcessed[13] = {0};
	double Lumi[13] = {0};
	...
	// for (Long64_t jentry = 0; jentry < 100; jentry++)
	{
		...
		nbytes += nb;
		// event processed per partonic pt bin
		for (int j = 0; j < 13; j++)
		{
		...
		for (int tr = 0; tr < fmaxpar1; tr++)
		{
			...
			for (int tr2 = tr + 1; tr2 < fmaxpar1; tr2++)
			{
				...
				if (Minv > Minv_Max || Minv < Minv_Min || fabs(eta_pair) > Eta_Cut || pT_pair > ptPair_Max || pT_pair < ptPair_Min)
					continue;
				// in the measured corss-section, events from lower partonic pt bins reconstructed at the higher mass region are removed as they  picked huge weight causing the mismatch in the data-embedding. This criteria is retained here as well.
				if (partonicPtBin < 5 && Minv > 1.5)
					continue;
	...
	// divide bin yields by the luminosity per partonic pt bin and add together
	for (int i = 0; i < 13; i++)
	{
		hevProcessed[i]->SetBinContent(1, eventProcessed[i]);
		Lumi[i] = eventProcessed[i] / ptXsecPb[i];
		hpythiaXsec[i]->Add(hpythiaMinv[i], 1. / Lumi[i]);
	}
	// combine all pt bins
	for (int i = 0; i < 13; i++)
	{
		hpythiaXsecAbs->Add(hpythiaXsec[i]);
	}

	for (int i = 1; i <= 13; i++)
	{
		double bw = hpythiaXsecAbs->GetBinWidth(i);
		double bc = hpythiaXsecAbs->GetBinContent(i);
		double be = hpythiaXsecAbs->GetBinError(i);

		hpythiaXsecAbs->SetBinContent(i, bc / bw);
		hpythiaXsecAbs->SetBinError(i, be);
	}
...
void Run12pp200Ana::Finish(char *ofile)
{
	...
	{
		hpythiaMinv[i]->Write();
		hevProcessed[i]->Write();
		hpythiaXsec[i]->Write();
	}
	hpythiaXsecAbs->Write();
