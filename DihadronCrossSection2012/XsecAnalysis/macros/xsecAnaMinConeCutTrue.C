...
void xsecAnaMinConeCutTrue()
{
    ...
    TFile *fxout = new TFile("ResultsMinConeCut/fxsecOutputTrue.root", "Recreate");
    ...
    // TFile *frawX = new TFile("PythiaXsec/pythiaXsecNoVZcut.root", "R");
    ...
    TFile *funfolding = new TFile("../BinResolutionAndUnfolding/UnfoldingResultsMinConeCut/unfoldingOutputTrue.root", "r");
    //  unfolded in |eta|<1
    ...
    ofstream fxsec0;
    fxsec0.open("ResultsMinConeCut/fxsec_jp0_numbers_true.txt");
    fxsec0 << "#Cross section by numbers as the correction progress" << endl;
    ofstream fxsec_err0;
    fxsec_err0.open("ResultsMinConeCut/fxsec_err_jp0_numbers_true.txt");
    fxsec_err0 << "#Cross section statistical error by numbers as the correction progress" << endl;
    ofstream fxsec1;
    fxsec1.open("ResultsMinConeCut/fxsec_jp1_numbers_true.txt");
    fxsec1 << "#Cross section by numbers as the correction progress" << endl;
    ofstream fxsec_err1;
    fxsec_err1.open("ResultsMinConeCut/fxsec_err_jp1_numbers_true.txt");
    fxsec_err1 << "#Cross section statistical error by numbers as the correction progress" << endl;
    ofstream fxsec2;
    fxsec2.open("ResultsMinConeCut/fxsec_jp2_numbers_true.txt");
    fxsec2 << "#Cross section by numbers as the correction progress" << endl;
    ofstream fxsec_err2;
    fxsec_err2.open("ResultsMinConeCut/fxsec_err_jp2_numbers_true.txt");
    fxsec_err2 << "#Cross section statistical error by numbers as the correction progress" << endl;

    ofstream fsyserr;
    fsyserr.open("ResultsMinConeCut/syserr_numbers_true.txt");
    fsyserr << "#systematic errors by numbers " << endl;
    ...
    for (int i = 0; i < nBins; i++)
    {
        ...
        // cout << i << "  " << spr_jp0 << " " << spr_jp1 << " " << spr_jp2 << endl;
        // double spr_avg = (spr_jp0 + spr_jp1 + spr_jp2) / 3.0;
        // double spr_avg = sqrt(spr_jp0 * spr_jp0 + spr_jp1 * spr_jp1 + spr_jp2 * spr_jp2);//added in quadrature
        double spr_avg = (w2_jp0 * spr_jp0 + w2_jp1 * spr_jp1 + w2_jp2 * spr_jp2) / (w2_jp0 + w2_jp1 + w2_jp2); // weighted average
        sys_trg_err[i] = spr_avg;

        // sys_pid_err[i] = sqrt(pow(sys_pid_jp0[i], 2) + pow(sys_pid_jp1[i], 2) + pow(sys_pid_jp0[i], 2) + pow(sys_pidloss[i], 2)); // f_fake & f_loss, added in quadrature. f_loss systematic is the effect in the final cross section due to change in response matrix with dEdx and dEdx&&True PID selection.
        sys_pid_err[i] = (w2_jp0 * sys_pid_jp0[i] + w2_jp1 * sys_pid_jp1[i] + w2_jp2 * sys_pid_jp0[i]) / ((w2_jp0 + w2_jp1 + w2_jp2));

        // total systematic error
        sys_err[i] = sqrt(pow(sys_trigBias[i], 2) + pow(sys_trg_err[i], 2) + pow(sys_pid_err[i], 2) + pow(sys_pidloss[i], 2));

        if (i == 0)
            fsyserr << "1. Bin No. 2. trigeff 3. f_fake 4. f_loss 5. trigger bias 6. sys_total" << endl;

        fsyserr << i << "&" << setprecision(3) << sys_trg_err[i] << "&" << setprecision(3) << sys_pid_err[i] << "&" << setprecision(3) << sys_pidloss[i] << "&" << setprecision(3) << sys_trigBias[i] << "&" << setprecision(3) << sys_err[i] << "\\\\" << endl;

        // print output at each steps
        double x0_bl = dN_jp0[i] / (binWidth[i] * L_jp0[i]);
        double Err0_bl = stat_err_jp0[i] / (binWidth[i] * L_jp0[i]);
        double x0_bl_eftg = dN_jp0[i] / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i]);
        double Err0_bl_eftg = stat_err_jp0[i] / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i]);
        double x0_bl_eftg_eftr = dN_jp0[i] / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);
        double Err0_bl_eftg_eftr = stat_err_jp0[i] / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);
        double ff_x0_bl_eftg_eftr = (dN_jp0[i] * fraction_trueJP0[i]) / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);
        double ff_Err0_bl_eftg_eftr = (stat_err_jp0[i] * fraction_trueJP0[i]) / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);
        double ff_fl_x0_bl_eftg_eftr = (dN_jp0[i] * fraction_trueJP0[i] * fraction_missedJP0[i]) / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);
        double ff_fl_Err0_bl_eftg_eftr = (stat_err_jp0[i] * fraction_trueJP0[i] * fraction_missedJP0[i]) / (binWidth[i] * L_jp0[i] * eff_trg_jp0[i] * trkeff_jp0[i]);

        if (i == 0)
            fxsec0 << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec0 << i << "&" << binWidth[i] << "&" << L_jp0 << "&" << setprecision(3) << eff_trg_jp0[i] << "&" << setprecision(3) << trkeff_jp0[i] << "&" << setprecision(3) << fraction_trueJP0[i] << "&" << setprecision(3) << fraction_missedJP0[i] << "&" << setprecision(3) << dN_jp0[i] << "&" << setprecision(3) << x0_bl << "&" << setprecision(3) << x0_bl_eftg << "&" << setprecision(3) << x0_bl_eftg_eftr << "&" << setprecision(3) << ff_x0_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_x0_bl_eftg_eftr << "\\\\" << endl;

        if (i == 0)
            fxsec_err0 << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec_err0 << i << "&" << binWidth[i] << "&" << L_jp0 << "&" << setprecision(3) << eff_trg_jp0[i] << "&" << setprecision(3) << trkeff_jp0[i] << "&" << setprecision(3) << fraction_trueJP0[i] << "&" << setprecision(3) << fraction_missedJP0[i] << "&" << setprecision(3) << stat_err_jp0[i] << "&" << setprecision(3) << Err0_bl << "&" << setprecision(3) << Err0_bl_eftg << "&" << setprecision(3) << Err0_bl_eftg_eftr << "&" << setprecision(3) << ff_Err0_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_Err0_bl_eftg_eftr << "\\\\" << endl;

        double x1_bl = dN_jp1[i] / (binWidth[i] * L_jp1[i]);
        double Err1_bl = stat_err_jp1[i] / (binWidth[i] * L_jp1[i]);
        double x1_bl_eftg = dN_jp1[i] / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i]);
        double Err1_bl_eftg = stat_err_jp1[i] / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i]);
        double x1_bl_eftg_eftr = dN_jp1[i] / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);
        double Err1_bl_eftg_eftr = stat_err_jp1[i] / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);
        double ff_x1_bl_eftg_eftr = (dN_jp1[i] * fraction_trueJP1[i]) / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);
        double ff_Err1_bl_eftg_eftr = (stat_err_jp1[i] * fraction_trueJP1[i]) / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);
        double ff_fl_x1_bl_eftg_eftr = (dN_jp1[i] * fraction_trueJP1[i] * fraction_missedJP1[i]) / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);
        double ff_fl_Err1_bl_eftg_eftr = (stat_err_jp1[i] * fraction_trueJP1[i] * fraction_missedJP1[i]) / (binWidth[i] * L_jp1[i] * eff_trg_jp1[i] * trkeff_jp1[i]);

        if (i == 0)
            fxsec1 << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec1 << i << "&" << binWidth[i] << "&" << L_jp1 << "&" << setprecision(3) << eff_trg_jp1[i] << "&" << setprecision(3) << trkeff_jp1[i] << "&" << setprecision(3) << fraction_trueJP1[i] << "&" << setprecision(3) << fraction_missedJP1[i] << "&" << setprecision(3) << dN_jp1[i] << "&" << setprecision(3) << x1_bl << "&" << setprecision(3) << x1_bl_eftg << "&" << setprecision(3) << x1_bl_eftg_eftr << "&" << setprecision(3) << ff_x1_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_x1_bl_eftg_eftr << "\\\\" << endl;

        if (i == 0)
            fxsec_err1
                << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec_err1 << i << "&" << binWidth[i] << "&" << L_jp1 << "&" << setprecision(3) << eff_trg_jp1[i] << "&" << setprecision(3) << trkeff_jp1[i] << "&" << setprecision(3) << fraction_trueJP1[i] << "&" << setprecision(3) << fraction_missedJP1[i] << "&" << setprecision(3) << stat_err_jp1[i] << "&" << setprecision(3) << Err1_bl << "&" << setprecision(3) << Err1_bl_eftg << "&" << setprecision(3) << Err1_bl_eftg_eftr << "&" << setprecision(3) << ff_Err1_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_Err1_bl_eftg_eftr << "\\\\" << endl;

        double x2_bl = dN_jp2[i] / (binWidth[i] * L_jp2[i]);
        double Err2_bl = stat_err_jp2[i] / (binWidth[i] * L_jp2[i]);
        double x2_bl_eftg = dN_jp2[i] / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i]);
        double Err2_bl_eftg = stat_err_jp2[i] / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i]);
        double x2_bl_eftg_eftr = dN_jp2[i] / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);
        double Err2_bl_eftg_eftr = stat_err_jp2[i] / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);
        double ff_x2_bl_eftg_eftr = (dN_jp2[i] * fraction_trueJP2[i]) / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);
        double ff_Err2_bl_eftg_eftr = (stat_err_jp2[i] * fraction_trueJP2[i]) / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);
        double ff_fl_x2_bl_eftg_eftr = (dN_jp2[i] * fraction_trueJP2[i] * fraction_missedJP2[i]) / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);
        double ff_fl_Err2_bl_eftg_eftr = (stat_err_jp2[i] * fraction_trueJP2[i] * fraction_missedJP2[i]) / (binWidth[i] * L_jp2[i] * eff_trg_jp2[i] * trkeff_jp2[i]);

        if (i == 0)
            fxsec2 << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec2 << i << "&" << binWidth[i] << "&" << L_jp2 << "&" << setprecision(3) << eff_trg_jp2[i] << "&" << setprecision(3) << trkeff_jp2[i] << "&" << setprecision(3) << fraction_trueJP2[i] << "&" << setprecision(3) << fraction_missedJP2[i] << "&" << setprecision(3) << dN_jp2[i] << "&" << setprecision(3) << x2_bl << "&" << setprecision(3) << x2_bl_eftg << "&" << setprecision(3) << x2_bl_eftg_eftr << "&" << setprecision(3) << ff_x2_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_x2_bl_eftg_eftr << "\\\\" << endl;

        if (i == 0)
            fxsec_err2 << "1. Bin No.2. BW 3. L 4. Etrg 5. Etrk 6.ff 7. fl 8. XUn.(Xerr) 9. Xerr/(BW*L) 9. XErr/(BW*L*Etrg) 10. XErr/(BW*L*Etrg*Etrk) 11. (XErr*ff)/(BW*L*Etrg*Etrk) 12. (XErr*ff*fl)/(BW*L*Etrg*Etrk)" << endl;

        fxsec_err2 << i << "&" << binWidth[i] << "&" << L_jp2 << "&" << setprecision(3) << eff_trg_jp2[i] << "&" << setprecision(3) << trkeff_jp2[i] << "&" << setprecision(3) << fraction_trueJP2[i] << "&" << setprecision(3) << fraction_missedJP2[i] << "&" << setprecision(3) << stat_err_jp2[i] << "&" << setprecision(3) << Err2_bl << "&" << setprecision(3) << Err2_bl_eftg << "&" << setprecision(3) << Err2_bl_eftg_eftr << "&" << setprecision(3) << ff_Err2_bl_eftg_eftr << "&" << setprecision(3) << ff_fl_Err2_bl_eftg_eftr << "\\\\" << endl;
    }
    // print
    ofstream fxsec;
    fxsec.open("ResultsMinConeCut/fxsec_values_true.txt");
    fxsec << "1. Bin Range 2. Xsec 3. StatErr 4. Xsec_Pyth 5. Pyth_Err 6. TrgEffErr  7. SysPID 8. Sys_TrgBias 9. Sys_Total " << endl;

    for (int i = 0; i < 13; i++)
    {
        // fxsec << nBinsEdges[i] << "-" << nBinsEdges[i + 1] << "&" << setprecision(3) << xsec_jp[i] << "&" << setprecision(3) << stat_err_jp[i] << "&" << setprecision(3) << xsec_pyth[i] << "&" << setprecision(3) << stat_err_pyth[i] << "&" << setprecision(3) << sys_trg_err[i] << "&" << setprecision(3) << sys_pid_err[i] << "&" << setprecision(3) << sys_trigBias[i] << "&" << setprecision(3) << sys_err[i] << "\\\\" << endl;
        fxsec << nBinsEdges[i] << "-" << nBinsEdges[i + 1] << "&" << setprecision(3) << xsec_jp[i] << "&" << setprecision(3) << stat_err_jp[i] << "&" << setprecision(3) << xsec_pyth[i] << "&" << setprecision(3) << stat_err_pyth[i] << "&" << setprecision(3) << sys_err[i] << "\\\\" << endl;
    }
    ofstream fout;
    fout.open("ResultsMinConeCut/xsec_numbers_true.txt");

    fout << "double xsec_mes[13]={";
    for (int i = 0; i < 13; i++)
    {
        if (i == 12)
            fout << xsec_jp[i] << "};" << endl;
    ...
    fout << "double xsec_staterr_mes[13]={";
    for (int i = 0; i < 13; i++)
    {
        if (i == 12)
            fout << stat_err_jp[i] << "};" << endl;
        else
            fout << stat_err_jp[i] << ", ";
    }
    fout << "double sys_trgeff[13]={";
    for (int i = 0; i < 13; i++)
    {
        if (i == 12)
            fout << sys_trg_err[i] << "};" << endl;
        else
            fout << sys_trg_err[i] << ", ";
    }
    fxsec << "------------------------------------------------------------------------------------------------------" << endl;
    ...
    for (int i = 0; i < nBinsU; i++)
    {
        ...
        hloss_err->SetBinError(i + 1, sys_pidloss[i - 1]);
    ...
    cmpt->SaveAs("ResultsMinConeCut/xsec_MoneyPlotWpidCorrection.pdf");
    cmpt->SaveAs("ResultsMinConeCut/xsec_MoneyPlotWpidCorrection.png");
    cmpt->SaveAs("ResultsMinConeCut/xsec_MoneyPlotWpidCorrection.eps");
    delete cmpt;

    TCanvas *cxsec = new TCanvas("cxsec", "", 0, 0, 700, 800);
    drawXsec(hxsec_comb, hxsec_pyth, hxsec_jp2, hxsec_jp1, hxsec_jp0, cxsec);
    cxsec->SaveAs("ResultsMinConeCut/xsec_all.pdf");
    cxsec->SaveAs("ResultsMinConeCut/xsec_all.png");
    cxsec->SaveAs("ResultsMinConeCut/xsec_all.eps");
    delete cxsec;
...
void drawXsec(TH1D *hcom, TH1D *hpyth, TH1D *hjp2, TH1D *hjp1, TH1D *hjp0, TCanvas *can)
{
    ...
    hpyth->GetYaxis()->SetTitle("#font[22]{#frac{d#sigma^{#pi^{+}#pi^{-}}}{dM^{#pi^{+}#pi^{-}}}(= #frac{f_{fake}*f_{loss}}{L #upoint  #epsilon_{trk}^{#pi^{+}} #upoint #epsilon_{trk}^{#pi^{-}} #upoint #epsilon_{trg}^{#pi^{+}#pi^{-}}} #times #frac{dN^{#pi^{+}#pi^{-}}_{true}}{dM_{inv}^{#pi^{+}#pi^{-}}})}");
    ...
    h_R2->GetYaxis()->SetTitle("#font[22]{#frac{#sigma_{mes} - #sigma_{pyth}}{#sigma_{pyth}}}");
    h_R2->GetXaxis()->SetTitle("#font[22]{M^{#pi^{+}#pi^{-}}_{inv} (GeV/c^{2})}");
    h_R2->GetYaxis()->CenterTitle();
    h_R2->GetYaxis()->SetRangeUser(-2, 2);
...
void drawXsecMoneyPlot(TH1D *hcom, TH1D *hpyth, TH1D *hsys_err, TH1D *hstat_err, TH1D *htotal_err, TCanvas *can)
{
    ...
    hcom->SetMarkerStyle(8);
    hcom->SetMarkerSize(0.5);
    hcom->SetMarkerColor(2);
    hcom->SetLineColor(2);
    hcom->SetLineWidth(2);
    hcom->Draw("E1 SAME");
    ...
    tex.DrawLatex(1.3, hpyth->GetMaximum() * 0.007, "#font[22]{0.27 < M_{inv}^{#pi^{+}#pi^{-}} < 4.0 (GeV/c^{2}),  L_{int} = 26 (pb)^{-1} }");
    ...
    hsys_err->GetYaxis()->SetRangeUser(-0.75, 0.75);
    hsys_err->GetYaxis()->SetLabelSize(0.11);
    hsys_err->GetYaxis()->SetTitleOffset(0.55);
    hsys_err->GetYaxis()->SetTitle("#font[22]{#frac{#sigma_{mes.} - #sigma_{pyth}}{#sigma_{pyth}}}");
    ...
    TLegend *lg1 = new TLegend(0.2, 0.85, 0.5, 0.95);
    lg1->SetNColumns(3);
    ...
    TLatex tex1;
    tex1.SetTextSize(0.095);
    tex1.DrawLatex(0.45, hsys_err->GetMinimum() + 0.25, "#font[12]{10\% luminosity measurement uncertainty not shown.}");
    // tex1.DrawLatex(0.5, -0.31, "#font[12]{10\% luminosity measurement uncertainty not shown.}");
...
void drawErrors(TH1D *htotal, TH1D *hsys, TH1D *hpid, TH1D *hloss, TH1D *htrgspr, TH1D *hstat, TH1D *hbias)
{
    ...
    htotal->GetYaxis()->SetRangeUser(-0.22, 0.22);
    htotal->SetFillColorAlpha(42, 1);
    htotal->Draw("E2");

    TLegend *leg = new TLegend(0.2, 0.85, 0.9, 0.9);
    leg->SetNColumns(7);
    leg->AddEntry(htotal, "#delta_{total}", "f");
    leg->Draw();
    cerr->Update();
    ...
    hsys->SetLineColor(2);
    hsys->Draw("E1 SAME");
    leg->AddEntry(hsys, "#delta_{sys}", "lep");
    leg->Draw();
    cerr->Update();
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys.pdf");
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys.png");
    ...
    leg->Draw();
    cerr->Update();
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid.pdf");
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid.png");
    ...
    leg->Draw();
    cerr->Update();
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid_stat_trgspr.pdf");
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid_stat_trgspr.png");
    ...
    leg->Draw();
    cerr->Update();
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid_trgspr_stat_bias.pdf");
    cerr->SaveAs("ResultsMinConeCut/sys_err_total_sys_pid_trgspr_stat_bias.png");
