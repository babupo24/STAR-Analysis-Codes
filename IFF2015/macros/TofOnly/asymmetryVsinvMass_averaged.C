...
void asymmetryVsinvMass_averaged(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/iffNtuplesFinalA.root"){
	...
	Output.open("AsymVsMinvTOFonly.txt");
	...
	for(Int_t pbin=0;pbin<5;pbin++)
	{
		...
		fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTOFonly.pdf(", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pT%i_Minvbin%i_etaGt_BLUETOFonly.pdf",pbin, m);
			//c1->SaveAs(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTOFonly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pT%i_Minvbin%i_etaGt_YELLOWTOFonly.pdf",pbin, m);
			//c1->SaveAs(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTOFonly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pT%i_Minvbin%i_etaLt_BLUETOFonly.pdf",pbin, m);
			//c1->Print(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTOFonly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pTbin%i_Minvbin%i_etaLt_YELLOWTOFonly.pdf",pbin, m);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTOFonly.pdf", pbin));
...
//print asymmetry and error in text file for relative difference calculation in main TpcOrTof macro.
for(int j=0; j<5; j++){
	ofstream agtout;
	agtout.open(Form("AutVsMinvTofOnlyGt_pT%i.txt", j));
	for(int i=0; i<9; i++){
...
for(int j=0; j<5; j++){
	ofstream altout;
	altout.open(Form("AutVsMinvTofOnlyLt_pT%i.txt", j));
	for(int i=0; i<9; i++){
  ...
  Output<< "double  AWerr_pT5Gt[9] ={"<<WerrA_pT5g[0]<<", "<<WerrA_pT5g[1]<<", "<<WerrA_pT5g[2]<<", "<<WerrA_pT5g[3]<<", "<<WerrA_pT5g[4]<<", "<<WerrA_pT5g[5]<<", "<<WerrA_pT5g[6]<<", "<<WerrA_pT5g[7]<<", "<<WerrA_pT5g[8]<<"}"<<endl;
	...
	*/
	...
	///////// Signal fractions ///////////////// 
	//pion
	double pairpurityMGt[5]={0.995685,0.99413,0.992819,0.981137,0.951615};
	double pairpurityMLt[5]={0.997217,0.995865,0.994613,0.982959,0.954391};
	//double pairpuritypTGt[5]={0.978901,0.98193,0.981563,0.980261,0.971465};
	//double pairpuritypTLt[5]={0.981487,0.984854,0.98391,0.982889,0.973322};
	//kaon+proton
	...
	ofstream ftable;
	ftable.open("ifftable_AutVsMinvTOFonly.txt");
...
myCanAvg->SaveAs("AsymVsMinvTOFcutTOFonly.pdf");
...
/*	//Forward Asymmetry BLUE and YELLOW
	...
	myCan->SaveAs("AsymVsMinv_EtaGt0_ForwardTOFonly.pdf");
	myCanA->SaveAs("AsymVsMinv_EtaGt0_AverageTOFonly.pdf");
	...
	//Backward Asymmetry BLUE and YELLOW
		...
		gPad->Update();
	cB->Update();
	cB->SaveAs("AsymmetryVsMinv_BackwardTOFonly.pdf");	
