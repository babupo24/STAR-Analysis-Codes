...
void integratedAsym_Minv_9bins(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/iffNtuplesFinalA.root")
{
	...
	*/	
	...
	ofstream altout;
	ofstream agtout;
	agtout.open("AutVsMinvIntTofOnlyGt.txt");
	for(int i=0; i<9; i++){
		agtout<<WavgA_Mg[i]<<" "<<WerrA_Mg[i]<<endl;
	}

	altout.open("AutVsMinvIntTofOnlyLt.txt");
	for(int i=0; i<9; i++){
		altout<<WavgA_Ml[i]<<" "<<WerrA_Ml[i]<<endl;
	}
	...
	TGraphErrors *Run15s = new TGraphErrors(9,avg_Mgt,WavgA_Mg,errxps,combSysGt);
