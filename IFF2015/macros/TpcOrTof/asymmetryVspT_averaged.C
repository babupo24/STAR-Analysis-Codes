...
Output.open("AsymVspTTPCOrTOF.txt");
	float eta_pair;
	float fspinconfig;
	float cone;
	float pT_pair;
		...
		*/
		...
		double pi = 3.14159265359;		
 		fitCanvas[mbin]=new TCanvas(Form("fitCanvas_MBin%i",mbin),"",650,550);
                fitCanvas[mbin]->Print(Form("FitPlots_Mbin_%i_AsymVspTTPCOrTOF.pdf(", mbin));
		...
		//cout <<"polB:"<<avgPolB<<", polY: "<<avgPolY<< ", a="<<a<<", b="<<b<<", B="<<B<<", C="<<C<<", D="<<D<<", E="<<E<<", Asym["<<ang<<"]="<<Asym[ang]<<", dA["<<ang<<"]="<<dA[ang]<<endl;	
			...
			//sprintf(name,"Minv%i_pTbin%i_etaGt_BLUETPCOrTOF.pdf",mbin, m);
			//c1->SaveAs(name);
                fitCanvas[mbin]->Print(Form("FitPlots_Mbin_%i_AsymVspTTPCOrTOF.pdf", mbin));
 ...
 //-------------------------------------------------------------------------
	...
	//print values in table format
	  ofstream ftable;
        ftable.open("ifftable_AutVspTTPCOrTOF.txt");
...
myCanAvg->SaveAs("AsymVsPtTOFcutTPCOrTOF.pdf");
...
/*
	...
	myCan->SaveAs("AsymVspT_Cone.7_ForwardTPCOrTOF.pdf");
	myCanA->SaveAs("AsymVspT_Cone.7_AverageTPCOrTOF.pdf");
	...
	//Backward Asymmetry BLUE and YELLOW
		...
		gPad->Update();
	cB->Update();
	cB->SaveAs("AsymmetryVspT_Cone.7_BackwardTPCOrTOF.pdf");
*/	
chi2f->Write();
chi2f->Close();
