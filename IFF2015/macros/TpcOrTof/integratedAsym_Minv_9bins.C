...
void integratedAsym_Minv_9bins(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/iffNtuplesFinalA.root")
{
	...
	Output.open("DataYieldsTPCOrTOF.txt");
	ofstream out;
	out.open("intAsymMinvTPCOrTOF.txt");
	...
	int nentries = (int)ntuple1f->GetEntries(); //entries of trees 
...
//print values in table format
  ofstream ftable;
        ftable.open("ifftable_AutVsMinvIntPtTPCOrTOF.txt");
 ...
 for(int i=0; i<9; i++){
	...
	double avg_pT7=5.25;
     ...
     myCanF->Update();
     myCanF->SaveAs("asymVsMinvIntpTTPCOrTOF.pdf");
//------------------------
}//main
