...
void asymmetryVsEta_9bin(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/dihadNtuples.root")
{
	...
	Output.open("AsymVsEtaTPConly.txt");

	//get the trees  
	TTree *ntuple1tpc = (TTree*)f->Get("ntuple1tpc"); 
	...
	int  nentries = (int)ntuple1tpc->GetEntries();
...
*/	
	...
	c1->Print("FitPlots_etaTPConly.pdf(");
	...
	*/
	...
	for (int pbin=0; pbin<9; pbin++){
   	...
   	combSys[pbin]=sqrt(pow(biasEta[pbin],2) + pow(pidEta[pbin],2));
                ...
                Output << "Sys_Eta[9]={"<<biasEta[0]<<", "<<biasEta[1]<< ","<<biasEta[2]<< ","<<biasEta[3]<<","<<biasEta[4]<<", "<<biasEta[5]<<", "<<biasEta[6]<<", "<<biasEta[7]<< ", "<<biasEta[8]<<"};"<<endl;
	        //print values in tabular format
	       ofstream ftable;
	       ftable.open("ifftable_AutVsEtaTPConly.txt");
	...
	myCanA->SaveAs("asymVsEtaTOFcutTPConly.pdf");	
	//----------------------------//
...
avg->Update();
	...
	leg2->Draw();
avg->Update();
avg->SaveAs("AsymVsEta_ConeLt7_9bin_AverageTPConly.pdf");

	myCan->SaveAs("AsymVsEta_ConeLt7_9binTPConly.pdf");
