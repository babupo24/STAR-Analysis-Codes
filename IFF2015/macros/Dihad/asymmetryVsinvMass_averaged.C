...
void setPad(int i);
ofstream Output;
void asymmetryVsinvMass_averaged(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/dihadNtuples.root"){
	...
	Output.open("AsymVsMinvTPConly.txt");
...
*/
	...
	*/	
		...
		*/
		...
		fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTPConly.pdf(", pbin));
		...
		//cout <<"polB:"<<avgPolB<<", polY: "<<avgPolY<< ", a="<<a<<", b="<<b<<", B="<<B<<", C="<<C<<", D="<<D<<", E="<<E<<", Asym["<<ang<<"]="<<Asym[ang]<<", dA["<<ang<<"]="<<dA[ang]<<endl;	
			...
			sprintf(name,"pT%i_Minvbin%i_etaGt_BLUETPConly.pdf",pbin, m);
			//c1->SaveAs(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTPConly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pT%i_Minvbin%i_etaGt_YELLOWTPConly.pdf",pbin, m);
			//c1->SaveAs(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTPConly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pT%i_Minvbin%i_etaLt_BLUETPConly.pdf",pbin, m);
			//c1->Print(name);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTPConly.pdf", pbin));
		...
		for(int m=0;m<9;m++)
		{
			...
			sprintf(name,"pTbin%i_Minvbin%i_etaLt_YELLOWTPConly.pdf",pbin, m);
			fitCanvas[pbin]->Print(Form("FitPlots_pTbin_%i_AsymVsMinvTPConly.pdf", pbin));
...
Output<< "double Sys_pTgt5[9]={"<<biasMgt5[0]<<", "<<biasMgt5[1]<<", "<<biasMgt5[2]<<", "<<biasMgt5[3]<<", "<<biasMgt5[4]<<", "<<biasMgt5[5]<<", "<<biasMgt5[6]<<", "<<biasMgt5[7]<<", "<<biasMgt5[8]<<"}"<<endl; 
	...
	ofstream ftable;
	ftable.open("ifftable_AutVsMinvTPConly.txt");
  ...
  for(int pad=0;pad<6; pad++ ){
	  ...
	  if(pad==5){
		  ...
		  gr_AvsMgts[pad] = new TGraphErrors(9,minvAvg,A_Mgt,errxts,trigSysGt);
		  ...
		  gr_AvsMlts[pad] = new TGraphErrors(9,minvAvg,A_Mlt,errxts,trigSysLt);//Backward average 
...
myCanAvg->SaveAs("AsymVsMinvTOFcutTPConly.pdf");
	...
	myCan->SaveAs("AsymVsMinv_EtaGt0_ForwardTPConly.pdf");
	myCanA->SaveAs("AsymVsMinv_EtaGt0_AverageTPConly.pdf");
	...
	//Backward Asymmetry BLUE and YELLOW
		...
		gPad->Update();
	cB->Update();
	cB->SaveAs("AsymmetryVsMinv_BackwardTPConly.pdf");	
