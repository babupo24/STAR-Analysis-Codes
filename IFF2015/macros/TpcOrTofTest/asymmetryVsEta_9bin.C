...
void asymmetryVsEta_9bin(const char *ifile="/star/u/pokhrel/GPFS/IFF_TREES/StartLessTOF/iffNtuplesFinalA.root")
{
	...
	Output.open("AsymVsEtaTPCOrTOF.txt");

	//get the trees  
	TTree *ntuple1f = (TTree*)f->Get("ntuple1f"); 
	...
	int  nentries = (int)ntuple1f->GetEntries();
...
Output<<"*******************  A_{UT vs #eta , cone < 0.7 , trigger: 1, 2 *************}"<<endl;
	...
	c1->Print("FitPlots_etaTPCOrTOF.pdf(");
	...
	*/
	...
	for (int pbin=0; pbin<9; pbin++){
   	...
   	combSys[pbin]=sqrt(pow(biasEta[pbin],2) + pow(pidEta[pbin],2));
	       ...
	       ofstream ftable;
	       ftable.open("ifftable_AutVsEtaTPCOrTOF.txt");
	...
	myCanA->SaveAs("asymVsEtaTOFcutTPCOrTOF.pdf");	
	//----------------------------//
	/*		
	gStyle->SetOptDate(0);
	gROOT->ForceStyle(0);
...
avg->Update();
	...
	leg2->Draw();
avg->Update();
avg->SaveAs("AsymVsEta_ConeLt7_9bin_AverageTPCOrTOF.pdf");

	myCan->SaveAs("AsymVsEta_ConeLt7_9binTPCOrTOF.pdf");
*/
hist4EtaBins->Write();
hist4EtaBins->Close();
}
